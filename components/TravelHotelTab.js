...
function TravelTable({ title, rows, onUpdate, onRemove, onAdd, sortField, sortDir, onSort, type }) {
  const GRID = '1fr 90px 100px 90px 130px 1fr 32px'
  ...
  const handleExportPDF = () => {
    ...
    win.document.write(`
      ...
      <h2>${title}</h2>
      <table><thead><tr><th>Name</th><th>Date</th><th>Flight #</th><th>Time</th><th>Transport</th><th>Notes</th></tr></thead>
    ...
    <div style={{ flex: 1, minWidth: 0 }}>
      ...
      <div className="glass-card" style={{ overflow: 'visible' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: GRID, gap: '0 10px', padding: '8px 14px', borderBottom: '0.5px solid var(--glass-border)' }}>
          ...
          <SortHeader label="Notes" field="notes" sortField={sortField} sortDir={sortDir} onSort={onSort} />
          <div />
        </div>
...
export default function TravelHotelTab({ eventId, event }) {
  ...
  const [roomStaffPicker, setRoomStaffPicker] = useState(null) // { roomId, slot }
  ...
  const fetchAll = async () => {
    ...
    const enrichArrivals = (arrRes.data || []).map(r => ({ ...r, staff_name: r.staff ? `${r.staff.first_name} ${r.staff.last_name}` : null }))
    const enrichDepartures = (depRes.data || []).map(r => ({ ...r, staff_name: r.staff ? `${r.staff.first_name} ${r.staff.last_name}` : null }))

    setArrivals(enrichArrivals)
    setDepartures(enrichDepartures)
  ...
  const handleSort = (table, field) => {
    if (table === 'arrival') {
      setArrivalSort(prev => ({ field, dir: prev.field === field && prev.dir === 'asc' ? 'desc' : 'asc' }))
    } else {
      setDepartureSort(prev => ({ field, dir: prev.field === field && prev.dir === 'asc' ? 'desc' : 'asc' }))
    }
  }

  const handleUpdateArrival = async (id, field, value) => {
    const supabase = getSupabase()
    await supabase.from('event_travel_arrivals').update({ [field]: value || null }).eq('id', id)
    fetchAll()
  }

  const handleUpdateDeparture = async (id, field, value) => {
    const supabase = getSupabase()
    await supabase.from('event_travel_departures').update({ [field]: value || null }).eq('id', id)
    fetchAll()
  }

  const handleRemoveArrival = async (id) => {
    const supabase = getSupabase()
    await supabase.from('event_travel_arrivals').delete().eq('id', id)
    fetchAll()
  }

  const handleRemoveDeparture = async (id) => {
    const supabase = getSupabase()
    await supabase.from('event_travel_departures').delete().eq('id', id)
    fetchAll()
  }

  const handleAddArrival = async (staffMember) => {
    const supabase = getSupabase()
    await supabase.from('event_travel_arrivals').insert([{ event_id: eventId, staff_id: staffMember.id }])
  ...
  const handleAddDeparture = async (staffMember) => {
    const supabase = getSupabase()
    await supabase.from('event_travel_departures').insert([{ event_id: eventId, staff_id: staffMember.id }])
  ...
  const handleSaveHotel = async () => {
    const supabase = getSupabase()
    if (hotel) {
      await supabase.from('event_hotel').update(hotelForm).eq('id', hotel.id)
    } else {
      await supabase.from('event_hotel').insert([{ ...hotelForm, event_id: eventId }])
    }
  ...
  const handleAddRoom = async () => {
    const supabase = getSupabase()
    await supabase.from('event_hotel_rooms').insert([{ event_id: eventId, room_type: 'Double' }])
    fetchAll()
  }

  const handleRemoveRoom = async (id) => {
    const supabase = getSupabase()
    await supabase.from('event_hotel_rooms').delete().eq('id', id)
    fetchAll()
  }

  const handleUpdateRoom = async (id, field, value) => {
    const supabase = getSupabase()
    await supabase.from('event_hotel_rooms').update({ [field]: value || null }).eq('id', id)
    fetchAll()
  }
  ...
  const handleAssignRoomStaff = async (staffMember) => {
    ...
    // Auto-fill check in/out from travel dates
    ...
    if (arrival?.travel_date || departure?.travel_date) {
      const updates = {}
      // Check in = earliest of both occupants' arrivals
      ...
      // Check out = latest of both occupants' departures
      ...
      if (Object.keys(updates).length > 0) {
        await supabase.from('event_hotel_rooms').update(updates).eq('id', roomId)
      }
    }
    fetchAll()
  }

  // Unroomed staff — confirmed staff not in any room
  ...
  const handleExportRoomingPDF = () => {
    ...
    const tableRows = rooms.map(r => `
      <tr>
        <td>${r.s1 ? `${r.s1.first_name} ${r.s1.last_name}` : '—'}</td>
        <td>${r.s2 ? `${r.s2.first_name} ${r.s2.last_name}` : '—'}</td>
        <td>${r.room_type || '—'}</td>
    ...
    `).join('')
    win.document.write(`
      <html><head><title>Rooming List</title>
      <style>body{font-family:sans-serif;padding:20px}h1{margin-bottom:4px}h3{color:#666;font-weight:normal;margin-bottom:20px}table{width:100%;border-collapse:collapse}th,td{border:1px solid #ccc;padding:8px;text-align:left}th{background:#f5f5f5}</style>
      </head><body>
      <h1>Rooming List</h1>
      ${hotel ? `<h3>${hotel.hotel_name || ''} · ${hotel.address || ''}</h3>` : ''}
      <table><thead><tr><th>Name 1</th><th>Name 2</th><th>Room Type</th><th>Check In</th><th>Check Out</th><th>Notes</th></tr></thead>
      <tbody>${tableRows}</tbody></table>
      </body></html>
    `)
    win.document.close()
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 28 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 24 }}>
        <TravelTable
          title="Arrivals"
          rows={sortRows(arrivals, arrivalSort)}
          onUpdate={handleUpdateArrival}
          onRemove={handleRemoveArrival}
          onAdd={() => setAddingArrival(true)}
        ...
        <TravelTable
          title="Departures"
          rows={sortRows(departures, departureSort)}
          onUpdate={handleUpdateDeparture}
          onRemove={handleRemoveDeparture}
          onAdd={() => setAddingDeparture(true)}
      ...
      <div>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            <button onClick={() => setEditingHotel(true)} style={{ fontFamily: 'Rubik, sans-serif', fontSize: 12, padding: '4px 10px', borderRadius: 6, border: '0.5px solid var(--glass-border)', background: 'transparent', color: 'var(--text-muted)', cursor: 'pointer' }}>
        ...
        {hotel && (
          <div className="glass-card" style={{ padding: '14px 18px', marginBottom: 16, display: 'flex', gap: 24 }}>
            <div>
              <div style={{ fontSize: 11, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.07em', marginBottom: 3 }}>Hotel</div>
              <div style={{ fontSize: 14, fontWeight: 500 }}>{hotel.hotel_name || '—'}</div>
            </div>
            {hotel.address && (
              <div>
                <div style={{ fontSize: 11, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.07em', marginBottom: 3 }}>Address</div>
                <div style={{ fontSize: 14 }}>{hotel.address}</div>
              </div>
            )}
            <div>
              <div style={{ fontSize: 11, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.07em', marginBottom: 3 }}>Check In</div>
              <div style={{ fontSize: 14 }}>{fmtDate(hotel.check_in_date)}</div>
            </div>
            <div>
              <div style={{ fontSize: 11, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.07em', marginBottom: 3 }}>Check Out</div>
              <div style={{ fontSize: 14 }}>{fmtDate(hotel.check_out_date)}</div>
            </div>
            {hotel.notes && (
              <div>
                <div style={{ fontSize: 11, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.07em', marginBottom: 3 }}>Notes</div>
                <div style={{ fontSize: 14 }}>{hotel.notes}</div>
              </div>
            )}
          </div>
        )}
        ...
        <div className="glass-card" style={{ overflow: 'visible' }}>
          ...
          {rooms.length === 0 && (
            <div style={{ padding: '16px 14px', fontSize: 13, color: 'var(--text-muted)' }}>No rooms added yet.</div>
          )}

          {rooms.map((room, i) => (
            <div key={room.id} style={{
              display: 'grid', gridTemplateColumns: ROOM_GRID, gap: '0 12px',
              padding: '10px 14px', alignItems: 'center',
              borderBottom: i < rooms.length - 1 ? '0.5px solid var(--glass-border)' : 'none',
            }}>
              {/* Name 1 */}
              <div onClick={() => setRoomStaffPicker({ roomId: room.id, slot: 'staff_id_1' })}
                style={{ fontSize: 13, cursor: 'pointer', color: room.s1 ? 'var(--text-primary)' : 'var(--mint)' }}
              ...
              {/* Name 2 */}
              <div onClick={() => setRoomStaffPicker({ roomId: room.id, slot: 'staff_id_2' })}
                style={{ fontSize: 13, cursor: 'pointer', color: room.s2 ? 'var(--text-primary)' : 'var(--text-muted)' }}
                onMouseEnter={e => e.currentTarget.style.opacity = '0.7'}
                onMouseLeave={e => e.currentTarget.style.opacity = '1'}
              >
              ...
              {/* Room Type */}
              <select
                value={room.room_type || 'Double'}
                onChange={e => handleUpdateRoom(room.id, 'room_type', e.target.value)}
                style={{ fontFamily: 'Rubik, sans-serif', fontSize: 12, padding: '3px 6px', borderRadius: 5, border: '0.5px solid var(--glass-border)', background: 'rgba(255,255,255,0.05)', color: 'var(--text-primary)', outline: 'none', cursor: 'pointer', width: '100%' }}
              >
              ...
              {/* Remove */}
              <div onClick={() => handleRemoveRoom(room.id)}
                style={{ fontSize: 16, color: 'var(--text-muted)', cursor: 'pointer', opacity: 0.4, textAlign: 'right' }}
                onMouseEnter={e => { e.currentTarget.style.color = 'var(--red)'; e.currentTarget.style.opacity = '1' }}
                onMouseLeave={e => { e.currentTarget.style.color = 'var(--text-muted)'; e.currentTarget.style.opacity = '0.4' }}
              >×</div>
            </div>
      ...
      {unroomedStaff.length > 0 && (
        <div>
          <div style={{ fontSize: 13, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.08em', color: 'var(--text-muted)', marginBottom: 12 }}>
            Unroomed Staff ({unroomedStaff.length})
          </div>
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
            {unroomedStaff.map(s => (
              <div key={s.id} className="glass-card" style={{ padding: '6px 14px', fontSize: 13 }}>
                {s.first_name} {s.last_name}
              </div>
            ))}
          </div>
          ...
          <div style={{ background: '#0d1f3a', border: '0.5px solid var(--glass-border)', borderRadius: 12, padding: 28, width: 500, display: 'flex', flexDirection: 'column', gap: 16 }} onClick={e => e.stopPropagation()}>
            ...
            {[
              ...
              <div key={f.key}>
                ...
                <input type={f.type}
                  style={{ fontFamily: 'Rubik, sans-serif', fontSize: 14, padding: '9px 12px', borderRadius: 7, border: '0.5px solid var(--glass-border)', background: 'rgba(255,255,255,0.05)', color: 'var(--text-primary)', outline: 'none', width: '100%' }}
                  value={hotelForm[f.key]}
                  onChange={e => setHotelForm(prev => ({ ...prev, [f.key]: e.target.value }))}
                />
              </div>
            ))}
