...
function isHatchedCell(posRow, event, eventMeta) {
  ...
  if (hidden.includes(posRow.key)) return true
...
function InlineStaffSearch({ eventId, event, onAssign, onClose, initialValue, allBookings }) {
  ...
  const handleKeyDown = (e) => {
    if (e.key === 'ArrowDown') { e.preventDefault(); if (optionCount > 0) setActiveIndex(i => (i + 1) % optionCount) }
    else if (e.key === 'ArrowUp') { e.preventDefault(); if (optionCount > 0) setActiveIndex(i => (i - 1 + optionCount) % optionCount) }
  ...
  return (
    <div onClick={e => e.stopPropagation()} style={{ position: 'relative', textAlign: 'left' }}>
      <input ref={inputRef} value={query} onChange={e => { const val = e.target.value; setQuery(val); setActiveIndex(-1); if (val.trim().length < 1) { setResults([]); setAvailability({}) } }} onKeyDown={handleKeyDown} autoComplete="off" placeholder="Type a name..."
      ...
      {showDropdown && (
        <div style={{ position: 'absolute', top: '100%', left: 0, zIndex: 1100, width: 200, background: '#0d1f3a', border: '0.5px solid rgba(255,255,255,0.12)', borderRadius: 8, marginTop: 4, maxHeight: 230, overflowY: 'auto', boxShadow: '0 8px 32px rgba(0,0,0,0.7)' }}>
          {loading && <div style={{ padding: '8px 10px', fontSize: 11, color: 'rgba(255,255,255,0.45)' }}>Searching...</div>}
          {!loading && results.length === 0 && <div style={{ padding: '8px 10px', fontSize: 11, color: 'rgba(255,255,255,0.45)' }}>No results</div>}
          {results.map((s, i) => {
            const status = availStatus(s.id)
            return (
              <div key={s.id} onMouseDown={e => { e.preventDefault(); selectIndex(i) }} onMouseEnter={() => setActiveIndex(i)} title={tipText(s.id)}
          ...
          <div onMouseDown={e => { e.preventDefault(); selectIndex(results.length) }} onMouseEnter={() => setActiveIndex(results.length)}
            style={{ padding: '7px 10px', fontSize: 11, cursor: 'pointer', color: '#33FF99', background: activeIndex === results.length ? 'rgba(51,255,153,0.1)' : 'transparent', borderTop: results.length > 0 ? '0.5px solid rgba(255,255,255,0.12)' : 'none' }}>
            + Create &quot;{query.trim()}&quot;
          </div>
        </div>
      )}
...
function InlineStatusMenu({ assignment, onSetStatus, onRemove }) {
  return (
    <div onClick={e => e.stopPropagation()} style={{ position: 'absolute', top: '100%', left: '50%', transform: 'translateX(-50%)', zIndex: 1100, width: 150, background: '#0d1f3a', border: '0.5px solid rgba(255,255,255,0.12)', borderRadius: 8, marginTop: 4, boxShadow: '0 8px 32px rgba(0,0,0,0.7)', overflow: 'hidden' }}>
      {STATUS_OPTIONS.map(opt => (
        <div key={opt.value} onMouseDown={e => { e.preventDefault(); onSetStatus(opt.value) }}
      ...
      <div onMouseDown={e => { e.preventDefault(); onRemove() }}
    ...
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.55)', zIndex: 9999, display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'Plus Jakarta Sans, sans-serif' }}>
      <div style={{ background: '#0d1f3a', border: '0.5px solid rgba(255,204,0,0.4)', borderRadius: 12, padding: 28, width: 440 }}>
        ...
        <div style={{ display: 'flex', gap: 12, justifyContent: 'flex-end' }}>
          <button onClick={onCancel} style={{ fontFamily: 'Plus Jakarta Sans, sans-serif', fontSize: 13, padding: '8px 16px', borderRadius: 7, border: '0.5px solid #33FF99', background: 'transparent', color: '#33FF99', cursor: 'pointer' }}
...
function GridCell({ eventId, event, positionRow, assignment, isHatched, onRefresh, onAssignSuccess, isActive, activeType, initialValue, isFocused, cellRef, onActivate, onCloseActive, onToggleLock, isSelected, onToggleSelect, onRightClick, cellColor, allBookings, onCellDragStart, onCellDragEnd, onCellDrop, onDragEnterCell, onDragLeaveCell, onDropOnLocked, isDragTarget, onClearAssignment, onConflictClick, onRefreshBookings, COL_WIDTH, ROW_HEIGHT, borderRight }) {
  ...
  const doAssign = async (staffMember, removeFromEventId = null) => {
    ...
    if (removeFromEventId) {
      await supabase.from('event_staff').update({ staff_id: null, status: 'pending', confirmed: false }).eq('event_id', removeFromEventId).eq('staff_id', staffMember.id)
      onClearAssignment && onClearAssignment(removeFromEventId, staffMember.id)
    }
    const status = isExec ? null : 'confirmed'
    const confirmed = status === 'confirmed'

    const { data: existingRows } = await supabase.from('event_staff').select('id').eq('event_id', eventId).eq('position_key', positionRow.key).limit(1)
    const existing = existingRows && existingRows[0]

    let writeError = null
    if (existing) {
      const { error } = await supabase.from('event_staff').update({ staff_id: staffMember.id, status, confirmed }).eq('id', existing.id)
      writeError = error
    } else {
      const { error } = await supabase.from('event_staff').insert([{ event_id: eventId, position: positionRow.displayLabel, position_key: positionRow.key, staff_id: staffMember.id, status, confirmed }])
      writeError = error
    }
    ...
    const { data: newRow } = await supabase.from('event_staff').select('id, event_id, staff_id, position_key, status').eq('event_id', eventId).eq('position_key', positionRow.key).single()
    if (newRow) onAssignSuccess(eventId, { ...localRow, id: newRow.id })

    onRefreshBookings && onRefreshBookings(staffMember.id)
  ...
  const handleAssign = async (staffMember, avail) => {
    if (avail && (avail.status === 'conflict' || avail.status === 'same_event')) {
      ...
      if (avail.status === 'conflict' && avail.event_id) {
        ...
        if (arrRes.data?.travel_date || depRes.data?.travel_date) travelInfo = { arrival: arrRes.data, departure: depRes.data }
      }
  ...
  const handleCellClick = (e) => {
    if (isActive) return
    if (cellState === 4) { e.stopPropagation(); onConflictClick && onConflictClick(assignment, existingConflict, event); return }
    if (cellState === 3 && (e.ctrlKey || e.metaKey)) { e.stopPropagation(); onToggleSelect(assignment.id); return }
  ...
  const stateBorder = cellState === 1 ? '1px solid rgba(255,255,255,0.08)' : cellState === 2 ? '2px solid #FFD60A' : cellState === 4 ? '2px solid #f87171' : null
  ...
  return (
    <React.Fragment>
      <td ref={cellRef} title={cellState === 1 ? 'Click to unlock' : undefined}
        style={{ position: 'relative', width: COL_WIDTH, minWidth: COL_WIDTH, maxWidth: COL_WIDTH, height: ROW_HEIGHT, padding: '0 8px', cursor: isActive ? 'default' : 'pointer', backgroundColor: stateBg, backgroundImage: cellState === 1 ? LOCKED_STRIPE : 'none', backgroundPosition: '0 0', textAlign: 'center', verticalAlign: 'middle', boxSizing: 'border-box', borderTop: stateBorder || undefined, borderLeft: stateBorder || undefined, borderRight: stateBorder || borderRight, borderBottom: stateBorder || '1px solid rgba(255,255,255,0.06)', boxShadow: assignError ? 'inset 0 0 0 1px #dc2626' : isDragOverLocked ? 'inset 0 0 0 1.5px #e05252' : (isDragTarget && cellState !== 1) ? 'inset 0 0 0 1.5px #33FF99' : (isFocused && !isActive) ? 'inset 0 0 0 1.5px rgba(255,255,255,0.4)' : 'none', zIndex: isActive ? 300 : (isFocused ? 5 : 1) }}
        ...
        onDragOver={e => { e.preventDefault(); e.dataTransfer.dropEffect = 'move'; onDragEnterCell && onDragEnterCell(eventId, positionRow.key) }}
        onDragLeave={e => { if (!e.currentTarget.contains(e.relatedTarget)) { onDragLeaveCell && onDragLeaveCell() } }}
        onDrop={e => { e.preventDefault(); if (isHatched) { onDropOnLocked && onDropOnLocked(eventId, positionRow.key) } else { onCellDrop && onCellDrop(eventId, positionRow.key) } }}>
        ...
        {isActive && activeType === 'edit' ? (
          ...
          <React.Fragment>
            ...
            <div onClick={e => { e.stopPropagation(); onToggleSelect(assignment.id) }}
  ...
  const handleCopy = async () => {
    ...
    for (const evId of checked) {
      for (const a of selectedRows) {
        try {
          const { data: existing } = await supabase.from('event_staff').select('id').eq('event_id', evId).eq('position_key', a.position_key).limit(1)
          if (existing && existing.length > 0) {
            await supabase.from('event_staff').update({ staff_id: a.staff_id, status: 'confirmed', confirmed: true }).eq('id', existing[0].id)
          } else {
            await supabase.from('event_staff').insert([{ event_id: evId, staff_id: a.staff_id, position: a.position, position_key: a.position_key, status: 'confirmed', confirmed: true }])
          }
        } catch (err) { console.error('[copy] error for', a.position_key, err) }
      }
    }
              ...
              {futureEvents.map(ev => {
                ...
                return (
                  <div key={ev.id} onClick={() => toggle(ev.id)}
                    style={{ padding: '12px 16px', borderRadius: 8, marginBottom: 8, cursor: 'pointer', border: '0.5px solid ' + (isChecked ? tourColor : 'rgba(255,255,255,0.08)'), background: isChecked ? colorWithAlpha(tourColor, 0.10) : 'rgba(255,255,255,0.04)' }}
                    ...
                    {tour && <div style={{ fontSize: 11, fontWeight: 600, color: tourColor, textTransform: 'uppercase', letterSpacing: '0.04em', marginBottom: 2 }}>{tour.name}</div>}
                    <div style={{ fontSize: 14, fontWeight: 500, color: '#f1f5f9' }}>{ev.city}{ev.country ? ', ' + ev.country : (ev.state ? ', ' + ev.state : '')}</div>
...
function BulkActionBar({ count, onSetStatus, onCopyToEvents, onClear }) {
  return (
    <div style={{ position: 'absolute', bottom: 24, left: '50%', transform: 'translateX(-50%)', zIndex: 300, background: '#1a2a42', border: '1px solid rgba(255,255,255,0.12)', borderRadius: 12, padding: '10px 16px', display: 'flex', alignItems: 'center', gap: 12, boxShadow: '0 8px 32px rgba(0,0,0,0.5)', whiteSpace: 'nowrap' }}>
      ...
      <div style={{ width: 1, height: 20, background: 'rgba(255,255,255,0.15)' }} />
      <button onClick={onCopyToEvents} style={{ fontFamily: 'Plus Jakarta Sans, sans-serif', fontSize: 12, padding: '5px 12px', borderRadius: 6, border: '0.5px solid #33FF99', background: 'transparent', color: '#33FF99', cursor: 'pointer' }}>Copy to Events</button>
      <div style={{ width: 1, height: 20, background: 'rgba(255,255,255,0.15)' }} />
      <button onClick={onClear} style={{ fontFamily: 'Plus Jakarta Sans, sans-serif', fontSize: 12, padding: '5px 10px', borderRadius: 6, border: '0.5px solid rgba(255,255,255,0.2)', background: 'transparent', color: 'rgba(255,255,255,0.5)', cursor: 'pointer' }}>Clear</button>
...
function AddPositionModal({ tourEvents, onAdd, onClose }) {
  const [dept, setDept] = useState(DEPARTMENT_ORDER[0])
  const [posName, setPosName] = useState('')
  const [saving, setSaving] = useState(false)

  const handleAdd = async () => {
    const name = posName.trim()
    if (!name || tourEvents.length === 0) return
    setSaving(true)
    const supabase = getSupabase()
    const key = 'custom__' + dept + '__' + name + '__1'
    for (const ev of tourEvents) {
      const { data: existing } = await supabase.from('event_staff').select('id').eq('event_id', ev.id).eq('position_key', key).limit(1)
      if (!existing || existing.length === 0) {
        await supabase.from('event_staff').insert([{ event_id: ev.id, position: name, position_key: key, confirmed: false, status: 'pending' }])
      }
    }
    setSaving(false)
    onAdd()
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.55)', zIndex: 2500, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <div style={{ background: '#0d1f3a', border: '1px solid rgba(255,255,255,0.12)', borderRadius: 16, padding: 28, width: 420, boxShadow: '0 20px 64px rgba(0,0,0,0.7)', fontFamily: 'Plus Jakarta Sans, sans-serif' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
          <div style={{ fontSize: 16, fontWeight: 700, color: '#f1f5f9' }}>Add Position</div>
          <div onClick={onClose} style={{ fontSize: 22, color: '#64748b', cursor: 'pointer', lineHeight: 1 }}>×</div>
        </div>
        <div style={{ marginBottom: 16 }}>
          <div style={{ fontSize: 11, fontWeight: 600, color: '#64748b', textTransform: 'uppercase', letterSpacing: '0.07em', marginBottom: 6 }}>Department</div>
          <select value={dept} onChange={e => setDept(e.target.value)}
            style={{ fontFamily: 'Plus Jakarta Sans, sans-serif', fontSize: 13, padding: '8px 12px', borderRadius: 8, border: '1px solid rgba(255,255,255,0.15)', background: 'rgba(255,255,255,0.08)', color: '#f1f5f9', width: '100%', outline: 'none', cursor: 'pointer' }}>
            {DEPARTMENT_ORDER.map(d => <option key={d} value={d}>{d}</option>)}
          </select>
        </div>
        <div style={{ marginBottom: 24 }}>
          <div style={{ fontSize: 11, fontWeight: 600, color: '#64748b', textTransform: 'uppercase', letterSpacing: '0.07em', marginBottom: 6 }}>Position Name</div>
          <input
            type="text" value={posName} onChange={e => setPosName(e.target.value)} placeholder="e.g. Camera Operator" autoFocus
            onKeyDown={e => { if (e.key === 'Enter') handleAdd(); if (e.key === 'Escape') onClose() }}
            style={{ fontFamily: 'Plus Jakarta Sans, sans-serif', fontSize: 13, padding: '8px 12px', borderRadius: 8, border: '1px solid rgba(255,255,255,0.15)', background: 'rgba(255,255,255,0.08)', color: '#f1f5f9', width: '100%', boxSizing: 'border-box', outline: 'none', caretColor: '#33FF99' }}
          />
        </div>
        <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end' }}>
          <button onClick={onClose} style={{ fontFamily: 'Plus Jakarta Sans, sans-serif', fontSize: 13, padding: '8px 16px', borderRadius: 8, border: '1px solid rgba(255,255,255,0.15)', background: 'transparent', color: '#94a3b8', cursor: 'pointer' }}>Cancel</button>
          <button onClick={handleAdd} disabled={saving || !posName.trim()}
            style={{ fontFamily: 'Plus Jakarta Sans, sans-serif', fontSize: 13, padding: '8px 18px', borderRadius: 8, border: 'none', background: !posName.trim() ? 'rgba(255,255,255,0.15)' : '#33FF99', color: !posName.trim() ? '#64748b' : '#0a1628', cursor: !posName.trim() ? 'default' : 'pointer', fontWeight: 600 }}>
...
export default function TourStaffingGrid({ tourId }) {
  ...
  const fetchAll = async () => {
    const supabase = getSupabase()
    const [toursRes, allEventsRes, assignmentsRes] = await Promise.all([
    ...
    const evData = allEventsRes.data || []
    setAllEvents(evData)
  ...
  useEffect(() => { fetchAll() }, [tourId])

  // Initialize year selector from this tour's events
  useEffect(() => {
    if (allEvents.length === 0 || selectedYear !== null) return
    const tourYears = [...new Set(
      allEvents.filter(e => e.tour_id === tourId && e.load_in_date)
        .map(e => new Date(e.load_in_date + 'T00:00:00').getFullYear())
    )].sort()
    if (tourYears.length > 0) {
      const currentYear = new Date().getFullYear()
      setSelectedYear(tourYears.includes(currentYear) ? currentYear : tourYears[tourYears.length - 1])
    }
  }, [allEvents, tourId, selectedYear])
  ...
  const getTourColor = (tid) => { const t = tours.find(t => t.id === tid); return t ? t.color : '#33FF99' }
  const getTourName = (tid) => { const t = tours.find(t => t.id === tid); return t ? t.name : '—' }
  ...
  const refreshBookingsForStaff = async (staffId) => {
    ...
    const { data } = await supabase.from('event_staff')
      ...
      .eq('staff_id', staffId)
    const bookings = (data || []).filter(a => a.events).map(a => {
      const { saturday_date, sunday_date } = getWeekendDates(a.events.load_in_date)
      return { event_id: a.event_id, city: a.events.city, load_in_date: a.events.load_in_date, load_out_date: a.events.load_out_date, saturday_date, sunday_date, tour_name: a.events.tours?.name, position: a.position }
    })
  ...
  const handleUnlockPosition = async (eventId, positionKey, positionRow) => {
    const supabase = getSupabase()
    const meta = eventMetas[eventId] || {}
    const hidden = meta.hidden_positions || []
    const unlocked = meta.unlocked_positions || []
  ...
  const handleToggleLock = (eventId, positionKey, isHatched, positionRow) => {
    if (isHatched) handleUnlockPosition(eventId, positionKey, positionRow)
  ...
  const handleClearStaffFromEvent = (eventId, staffId) => {
    setAssignments(prev => {
      const next = { ...prev }
      if (next[eventId]) next[eventId] = next[eventId].map(a => a.staff_id === staffId ? { ...a, staff_id: null, staff: null, status: 'pending', confirmed: false } : a)
      return next
    })
    setStaffBookingsMap(prev => ({ ...prev, [staffId]: (prev[staffId] || []).filter(b => b.event_id !== eventId) }))
  }
  ...
  const handleKeepHere = async (keepEventId, removeEventId) => {
    ...
    const supabase = getSupabase()
    await supabase.from('event_staff').update({ staff_id: null, status: 'pending', confirmed: false }).eq('event_id', removeEventId).eq('staff_id', staffId)
  ...
  const handleCellDragStart = (eventId, positionKey, assignment) => setDraggedCell({ eventId, positionKey, assignment })
  const handleCellDragEnd = () => { setDraggedCell(null); setDragOverCell(null) }
  const handleCellDragEnter = (eventId, positionKey) => setDragOverCell({ eventId, positionKey })
  ...
  const handleDropOnLocked = (eventId, positionKey) => {
    ...
    setDraggedCell(null); setDragOverCell(null)
  }
  ...
  const handleCellDrop = async (toEventId, toPosKey) => {
    ...
    if (toAssignment && toAssignment.staff_id) { setDraggedCell(null); return }
  ...
  const handleBulkStatus = async (status) => {
    ...
    if (confirmed) {
      ...
      await Promise.all(selectedRows.map(a => confirmStaffMember({ supabase, eventId: a.event_id, staffId: a.staff_id, confirm: true }).then(({ error }) => { if (error) console.error('Travel bulk sync failed:', error) })))
    }
    setAssignments(prev => {
      const next = { ...prev }
      for (const eid of Object.keys(next)) next[eid] = next[eid].map(a => ids.includes(a.id) ? { ...a, status, confirmed } : a)
      return next
    })
    setSelectedIds(new Set())
  }

  const handleSetCellBg = (eventId, positionKey, bg) => { const k = eventId + '__' + positionKey; setCellColors(prev => ({ ...prev, [k]: { ...prev[k], bg } })) }
  const handleSetCellText = (eventId, positionKey, text) => { const k = eventId + '__' + positionKey; setCellColors(prev => ({ ...prev, [k]: { ...prev[k], text } })) }
  ...
  const getPositionRowsForDept = (dept) => {
    ...
    for (const eventId of Object.keys(customPositions)) {
      for (const cp of (customPositions[eventId] || [])) {
        if (cp.dept === dept && !seenKeys.has(cp.key)) { seenKeys.add(cp.key); customRows.push(cp) }
      }
    }
    return [...templateRows, ...customRows]
  }

  // --- Build display events (this tour, year-filtered, past toggle) ---
  const today = toYMD(new Date())

  const tourEvents = allEvents.filter(e => e.tour_id === tourId)

  const availableYears = [...new Set(tourEvents.filter(e => e.load_in_date).map(e => new Date(e.load_in_date + 'T00:00:00').getFullYear()))].sort()

  const yearFilteredEvents = selectedYear
    ? tourEvents.filter(ev => ev.load_in_date && new Date(ev.load_in_date + 'T00:00:00').getFullYear() === selectedYear)
    : tourEvents

  const pastCount = yearFilteredEvents.filter(ev => (ev.load_out_date || ev.load_in_date) < today).length
  ...
  const isLastInGroup = (ev) => {
    ...
    return grp && grp[grp.length - 1].id === ev.id
  }
  ...
  useEffect(() => {
    ...
    const handleMouseDown = (e) => { if (activeCellElRef.current && !activeCellElRef.current.contains(e.target)) setActiveCell(null) }
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (e) => {
      ...
      if (activeCell) { if (e.key === 'Escape') { e.preventDefault(); setActiveCell(null); setSelectedIds(new Set()) }; return }
      ...
      switch (e.key) {
        ...
        default: {
          if (/^[a-zA-Z0-9]$/.test(e.key)) {
            ...
            const ev = orderedEvents.find(e2 => e2.id === focusedCell.eventId)
            if (!posRow || !ev) return
            const hatched = isHatchedCell(posRow, ev, eventMetas[ev.id] || {})
            const assignment = (assignments[ev.id] || []).find(a => a.position_key === posRow.key)
            if (hatched && !assignment) return
            e.preventDefault()
  ...
  if (loading) {
    return <div style={{ height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'rgba(255,255,255,0.45)', fontSize: 14, fontFamily: 'Plus Jakarta Sans, sans-serif' }}>Loading staffing grid…</div>
  }

  return (
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column', fontFamily: 'Plus Jakarta Sans, sans-serif', position: 'relative' }}>

      {/* Header bar */}
      <div style={{ flexShrink: 0, padding: '12px 20px', borderBottom: '0.5px solid rgba(255,255,255,0.08)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 16 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          {availableYears.length > 1 && (
            <select value={selectedYear || ''} onChange={e => setSelectedYear(Number(e.target.value))}
              style={{ fontFamily: 'Plus Jakarta Sans, sans-serif', fontSize: 13, padding: '5px 10px', borderRadius: 7, border: '1px solid rgba(255,255,255,0.15)', background: 'rgba(255,255,255,0.08)', color: '#f1f5f9', cursor: 'pointer', outline: 'none' }}>
              {availableYears.map(y => <option key={y} value={y}>{y}</option>)}
            </select>
          )}
          {pastCount > 0 && (
            <button onClick={() => setShowPast(p => !p)}
              style={{ fontFamily: 'Plus Jakarta Sans, sans-serif', fontSize: 12, padding: '5px 12px', borderRadius: 7, border: '0.5px solid rgba(255,255,255,0.15)', background: showPast ? 'rgba(255,255,255,0.08)' : 'transparent', color: showPast ? '#f1f5f9' : 'rgba(255,255,255,0.45)', cursor: 'pointer' }}>
              {showPast ? 'Hide Past Events' : 'Show Past Events (' + pastCount + ')'}
            </button>
          )}
        </div>
        <button onClick={() => setAddPositionOpen(true)}
          style={{ fontFamily: 'Plus Jakarta Sans, sans-serif', fontSize: 13, fontWeight: 600, padding: '6px 14px', borderRadius: 8, border: 'none', background: '#FFD60A', color: '#0a1628', cursor: 'pointer' }}
      ...
      {orderedEvents.length === 0 ? (
        <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', gap: 12, color: 'rgba(255,255,255,0.45)', fontSize: 14 }}>
          {tourEvents.length === 0 ? 'No events on this tour yet.' : 'No events to display. Try showing past events.'}
        </div>
      ) : (
        <div style={{ flex: 1, overflow: 'hidden', margin: '12px 16px 16px', borderRadius: 12, border: '1px solid rgba(255,255,255,0.10)' }}>
              ...
              <thead>
                <tr>
                  ...
                  {weekendGroups.map((wk, wi) => {
                    ...
                    return (
                      <th key={wk} colSpan={wkEvs.length} style={{ position: 'sticky', top: 0, zIndex: 30, height: H1, background: '#0d1f3c', borderBottom: B_HDR_INNER, borderRight: wi < weekendGroups.length - 1 ? B_HDR_WEEKEND : B_HDR_INNER, textAlign: 'center', fontWeight: 400, padding: '6px 0' }}>
                        <div style={{ fontSize: 10, fontWeight: 700, color: '#64748b', textTransform: 'uppercase', letterSpacing: '0.04em' }}>Weekend</div>
                        <div style={{ fontSize: 12, fontWeight: 600, color: '#94a3b8', marginTop: 2 }}>{fmtWeekend(wk)}</div>
                      </th>
                    )
                  })}
                </tr>
                  ...
                  {orderedEvents.map((ev, i) => {
                    ...
                    return (
                      <th key={ev.id} style={{ position: 'sticky', top: H1 + H2, zIndex: 30, width: COL_WIDTH, minWidth: COL_WIDTH, height: H3, background: DEPT_BG, borderBottom: B_HDR_INNER, borderRight: cellBorderRightDark(ev, i), padding: '0 6px', textAlign: 'center', fontWeight: 400 }}>
                        <span style={{ display: 'inline-block', fontSize: 10, fontWeight: 600, padding: '2px 8px', borderRadius: 20, border: '0.5px solid ' + pillBorder, background: pillBg, color: statusColor }}>{formatStatusLabel(ev.status)}</span>
                      </th>
                    )
                  })}
                </tr>
                <tr>
                  <th style={{ position: 'sticky', top: H1 + H2 + H3, left: 0, zIndex: 50, width: LEFT_WIDTH, minWidth: LEFT_WIDTH, height: H4, background: '#111827', borderRight: B_LEFT_COL, borderBottom: B_HDR_INNER, padding: '0 14px', textAlign: 'left', verticalAlign: 'middle' }}>
                    <span style={{ fontSize: 11, fontWeight: 700, color: '#64748b', textTransform: 'uppercase', letterSpacing: '0.06em' }}>Load-In</span>
                  </th>
                  {orderedEvents.map((ev, i) => (
                    <th key={ev.id} style={{ position: 'sticky', top: H1 + H2 + H3, zIndex: 30, width: COL_WIDTH, minWidth: COL_WIDTH, height: H4, background: HDR_BG, borderBottom: B_HDR_INNER, borderRight: cellBorderRightDark(ev, i), padding: '0 6px', textAlign: 'center', fontWeight: 400 }}>
                      <span style={{ fontSize: 11, color: ev.load_in_date ? '#94a3b8' : '#64748b' }}>{ev.load_in_date ? new Date(ev.load_in_date + 'T00:00:00').toLocaleDateString('en-US', { month: 'short', day: 'numeric' }) : '—'}</span>
                    </th>
                  ))}
                </tr>
                <tr>
                  ...
                  {orderedEvents.map((ev, i) => (
                    <th key={ev.id} style={{ position: 'sticky', top: H1 + H2 + H3 + H4, zIndex: 30, width: COL_WIDTH, minWidth: COL_WIDTH, height: H5, background: HDR_BG, borderBottom: '5px solid #FFD60A', borderRight: cellBorderRightDark(ev, i), padding: '0 6px', textAlign: 'center', fontWeight: 400, fontSize: 11, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                      <span style={{ color: ev.venue_name ? '#f1f5f9' : '#64748b' }}>{ev.venue_name || '—'}</span>
                    </th>
                  ))}
                </tr>
              </thead>
                  ...
                  return (
                    <React.Fragment key={dept}>
                      ...
                      {!collapsed && deptRows.map(posRow => {
                        return (
                          <tr key={posRow.key}>
                            <td style={{ position: 'sticky', left: 0, zIndex: 10, width: LEFT_WIDTH, minWidth: LEFT_WIDTH, height: ROW_HEIGHT, padding: '0 10px 0 6px', background: '#111827', borderRight: B_LEFT_COL, borderBottom: '1px solid rgba(255,255,255,0.06)', fontSize: 13, fontWeight: 500, color: '#e2e8f0', whiteSpace: 'nowrap', willChange: 'transform' }}>
                            ...
                            {orderedEvents.map((ev, i) => {
                              ...
                              return (
                                <GridCell
                                  ...
                                  onToggleLock={() => handleToggleLock(ev.id, posRow.key, hatched, posRow)}
                                  ...
                                  onRefreshBookings={refreshBookingsForStaff}
                                  COL_WIDTH={COL_WIDTH}
      ...
      {rightClickMenu && (
        <RightClickMenu x={rightClickMenu.x} y={rightClickMenu.y} eventId={rightClickMenu.eventId} positionKey={rightClickMenu.positionKey} onSetBg={handleSetCellBg} onSetText={handleSetCellText} onClose={() => setRightClickMenu(null)} />
      )}
      ...
      {addPositionOpen && (
        <AddPositionModal tourEvents={orderedEvents} onAdd={fetchAll} onClose={() => setAddPositionOpen(false)} />
      )}

      {toast && (
        <div style={{ position: 'absolute', bottom: 80, left: '50%', transform: 'translateX(-50%)', zIndex: 4000, background: '#33FF99', color: '#0a1628', borderRadius: 10, padding: '10px 20px', fontSize: 13, fontWeight: 600, boxShadow: '0 4px 20px rgba(0,0,0,0.4)', whiteSpace: 'nowrap' }}>
          ✓ {toast}
        </div>
      )}
      ...
      {dragLockedModal && typeof document !== 'undefined' && ReactDOM.createPortal(
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.55)', zIndex: 9999, display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'Plus Jakarta Sans, sans-serif' }}>
          <div style={{ background: '#0d1f3a', border: '0.5px solid rgba(224,82,82,0.4)', borderRadius: 12, padding: 28, width: 420 }}>
            ...
            <div style={{ display: 'flex', gap: 12, justifyContent: 'flex-end' }}>
              <button onClick={() => setDragLockedModal(null)} style={{ fontFamily: 'Plus Jakarta Sans, sans-serif', fontSize: 13, padding: '8px 16px', borderRadius: 7, border: '0.5px solid #33FF99', background: 'transparent', color: '#33FF99', cursor: 'pointer' }}
                onMouseEnter={e => e.currentTarget.style.background = 'rgba(51,255,153,0.08)'}
                onMouseLeave={e => e.currentTarget.style.background = 'transparent'}>Cancel</button>
              <button onClick={async () => {
                const { eventId, positionKey, draggedCell: dc } = dragLockedModal
                if (dc?.assignment?.staff) { await handleUnlockAndAssign(eventId, positionKey, dc.assignment.staff) }
                else { const posRow = MASTER_POSITIONS.find(p => p.key === positionKey); if (posRow) handleUnlockPosition(eventId, positionKey, posRow) }
      ...
      {conflictModal && typeof document !== 'undefined' && ReactDOM.createPortal(
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.65)', zIndex: 9999, display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'Plus Jakarta Sans, sans-serif' }}>
          <div style={{ background: '#0d1f3a', border: '0.5px solid rgba(255,214,10,0.35)', borderRadius: 14, padding: 32, width: 560, maxWidth: '95vw' }}>
            <div style={{ fontSize: 16, fontWeight: 700, color: '#FFD60A', marginBottom: 6 }}>Scheduling Conflict — {conflictModal.staffName}</div>
            <div style={{ fontSize: 13, color: '#94a3b8', marginBottom: 24 }}>{conflictModal.staffName} is assigned to two events on the same weekend. Choose which event to keep them on.</div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 24 }}>
              {[conflictModal.current, conflictModal.conflict].map((ev, idx) => (
                <div key={idx} style={{ background: 'rgba(255,255,255,0.05)', border: '0.5px solid rgba(255,255,255,0.12)', borderRadius: 10, padding: 18, display: 'flex', flexDirection: 'column', gap: 10 }}>
                  ...
                  <button onClick={() => handleKeepHere(ev.eventId, idx === 0 ? conflictModal.conflict.eventId : conflictModal.current.eventId)}
                    ...
                    onMouseLeave={e => e.currentTarget.style.background = '#33FF99'}>Keep Here</button>
                </div>
              ))}
            </div>
            <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
              <button onClick={() => setConflictModal(null)} style={{ fontFamily: 'Plus Jakarta Sans, sans-serif', fontSize: 13, padding: '8px 16px', borderRadius: 7, border: '0.5px solid rgba(255,255,255,0.2)', background: 'transparent', color: 'rgba(255,255,255,0.6)', cursor: 'pointer' }}>Dismiss</button>
            </div>
          </div>
        </div>,
        document.body
